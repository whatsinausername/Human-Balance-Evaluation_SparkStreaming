...
zDFencodedSetEntries=spark.sql("select zSetEntries[0].element as encodedCustomer from RedisSortedSet")
...
zDFdecodedSetEntries= zDFencodedSetEntries.withColumn("customer", unbase64(zDFencodedSetEntries.encodedCustomer).cast("string"))
...
zDFdecodedSetEntries.withColumn("customer", from_json("customer", SchemaCustomer))\
                             .select(col('customer.*'))\
                             .createOrReplaceTempView("CustomerRecords")
# TO-DO: JSON parsing will set non-existent fields to null, so let's select just the fields we want, where they are not null as a new dataframe called emailAndBircustomerSchema = StructType (
...
# +--------------------+-----
DFemailAndBirthYear.writeStream.outputMode("append").format("console").start().awaitTermination()
...
# kafka-console-consumer --topic "stedi-score" --bootstrap-server localhost:9092
