#reference https://github.com/Gares95/Human-Balance-Evaluation_Spark-Streaming
# https://github.com/chaitanyakasaraneni/evaluateHumanBalance
...
spark = SparkSession.builder.appName("Spark.app.STEDI").getOrCreate()
...
DFkafkaRedis = DFkafkaRedis.selectExpr("cast(value as string) value")
...
DFkafkaRedis.withColumn("value", from_json("value", SchemaRedisMessage))\
...
zDFSetEntriesEncoded =spark.sql("select zSetEntries[0].element as encodedCustomer from RedisSortedSet")
...
zDFSetEntriesEncoded=zDFSetEntriesEncoded.withColumn("customer", unbase64(zDFSetEntriesEncoded.encodedCustomer).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords

zDFSetEntriesEncoded.withColumn("customer", from_json("customer", SchemaCustomer))\
                             .select(col('customer.*'))\
...
DFemailAndBirthDay = spark.sql("select * from CustomerRecords where email is not null AND birthDay is not null")
...
DFemailAndBirthDay = DFemailAndBirthDay.withColumn('birthYear', split(DFemailAndBirthDay.birthDay,"-").getItem(0))

DFemailAndBirthYear = DFemailAndBirthDay.select(col('email'), col('birthYear'))
...
DFkafkaEvents = spark\
    ...
    .load()                                    
# TO-DO: cast the value column in the streaming dataframe as a STRING 
DFkafkaEvents = DFkafkaEvents.selectExpr("cast(value as string) value")
